...
class TTTMenu:
    ...
    def __init__(self, screen, item_size, pos, pad, color):
        ...
        self.pad = pad
        self.color = color
        self.w = self.item_size + 2 * self.pad
        self.h = 4 * (self.item_size + 4 * self.pad)
        
        self.items = ["rot_minus", "rot_plus", "cnot", "measure"]
    ...
    def draw(self):
        ...
        for row in range(4):
            ...
            if self.items[row] in self.highlighted:
                color = HIGHLIGHT
            elif self.mouseover_item == self.items[row]:
                color = MOUSEOVER
            else:
                color = self.color
            pg.draw.rect(self.screen, color, (x, y, sq_size, sq_size))
            image = pg.image.load(self.items[row] + ".png").convert_alpha()
